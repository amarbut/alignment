...
def select_and_save_cpca(cfg, model_base, harmful_acts, harmless_acts, topk = 3, keep_var = 0.999, eig_floor_frac = 1e-3):
    if not os.path.exists(os.path.join(cfg.artifact_path(), 'select_cpca')):
        os.makedirs(os.path.join(cfg.artifact_path(), 'select_cpca'))

    layer, pos, res = choose_best_cpca(harmful_acts, harmless_acts, topk, keep_var, eig_floor_frac)
...
def run_pipeline(model_path, skip_generate = False):
    ...
    if skip_generate:
        hl_acts = torch.load(os.path.join(cfg.artifact_path(), "generate_acts/hl_activations.pt"), map_location = "cpu")
        hf_acts = torch.load(os.path.join(cfg.artifact_path(), "generate_acts/hf_activations.pt"), map_location = "cpu")
    else:
