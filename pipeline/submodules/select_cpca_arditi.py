...
from pipeline.submodules.cpca_functions import cpca_whitened, get_lp_slices, align_sign_by_means
...
def select_cpca_with_arditi(
    ...
    """
    Compute cPCA at each layer/position, then select the best using Arditi's empirical criteria.
    ...
    Args:
        ...
        topk: Number of cPCA components to extract (default 1 for single direction)
    ...
    # Storage for cPCA results
    cpca_results = {}  # key: (pos, layer), value: cpca result dict

    print("Computing cPCA and evaluating with Arditi criteria...")

    # Compute cPCA and evaluate at each position/layer
    for source_pos in range(-n_pos, 0):
        ...
        for source_layer in tqdm(range(n_layer), desc=f"Position {source_pos}"):
            ...
            # Store result
            cpca_results[(source_pos, source_layer)] = res

            # Extract top-k components as direction(s)
            direction = res["components_norm"][:, :topk].to(model_base.model.device, dtype=torch.float32)  # [d_model, topk]
    ...
    plot_refusal_scores(
        ...
        title='Ablating cPCA direction on harmful instructions',
        artifact_dir=artifact_dir,
        artifact_name='cpca_ablation_scores'
    )
    ...
    plot_refusal_scores(
        ...
        token_labels=model_base.tokenizer.batch_decode(model_base.eoi_toks),
        title='Adding cPCA direction on harmless instructions',
        artifact_dir=artifact_dir,
        artifact_name='cpca_actadd_scores'
    )
    ...
    plot_refusal_scores(
        ...
        token_labels=model_base.tokenizer.batch_decode(model_base.eoi_toks),
        title='KL Divergence when ablating cPCA direction on harmless instructions',
        artifact_dir=artifact_dir,
        artifact_name='cpca_kl_div_scores'
    )
    ...
    # Save scores
    with open(f"{artifact_dir}/cpca_direction_evaluations.json", 'w') as f:
    ...
    with open(f"{artifact_dir}/cpca_direction_evaluations_filtered.json", 'w') as f:
    ...
    print(f"\nSelected cPCA direction: position={pos}, layer={layer}")
    ...
    # Return selected cPCA result
    selected_res = cpca_results[(pos, layer)]
    components = selected_res["components_norm"][:, :topk]
